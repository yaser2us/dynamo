...
function useHistory(init) {
  ...
  var _useState4 = React.useState((_init = init) === null || _init === void 0 ? void 0 : _init.name),
...
var useDynamoHistory = function useDynamoHistory(initialArr, field, id, preventDuplicates, replaceDuplicate) {
  ...
  React.useEffect(function () {
    if (history.length === 0) {
      ...
      var newIndex = history.findIndex(function (e) {
        return e[field] === id;
      });
  ...
  React.useEffect(function () {
    if (history.length > 0 && !history[0].type) {
      setHistory(function (prevHistory) {
        var updatedFirstObj = _extends({}, prevHistory[0], {
          type: 'primary'
        });
        return [updatedFirstObj].concat(prevHistory.slice(1));
      });
    }
  }, [history]);
  var getNextIndex = function getNextIndex() {
    return currentIndex + 1 >= history.length ? false : currentIndex + 1;
  };
  var getPreviousIndex = function getPreviousIndex() {
    return currentIndex - 1 < 0 ? false : currentIndex - 1;
  };
  ...
  var current = function current() {
    return currentIndex >= 0 && currentIndex < history.length ? history[currentIndex] : false;
  };
  ...
  var goTo = function goTo(newId, removeAfter) {
    if (removeAfter === void 0) {
      removeAfter = false;
    }
    if (history.length === 0) return;
    var newIndex = history.findIndex(function (e) {
      return e[field] === newId;
    });
    if (newIndex >= 0) {
  ...
  var goToIndex = function goToIndex(index, removeAfter) {
    if (removeAfter === void 0) {
      removeAfter = false;
    }
    if (history.length === 0) return;
  ...
  var goBack = function goBack(removeAfter) {
    if (removeAfter === void 0) {
      removeAfter = false;
    }
    if (history.length === 0) return;
  ...
  var goBackToPrimary = function goBackToPrimary(removeAfter, skipSecondary) {
    if (removeAfter === void 0) {
      removeAfter = false;
    }
    if (skipSecondary === void 0) {
      skipSecondary = true;
    }
    if (history.length === 0) return;
  ...
  var goForwardToType = function goForwardToType(removeAfter, skipSecondary, type) {
    if (removeAfter === void 0) {
      removeAfter = false;
    }
    if (skipSecondary === void 0) {
  ...
  var insertObjectAtIndex = function insertObjectAtIndex(object, insertIndex) {
    if (insertIndex > history.length) return;
    var newArray = [].concat(history);
    newArray.splice(insertIndex, 0, object);
    setHistory(newArray);
    setCurrentIndex(insertIndex);
  };
  var insertObject = function insertObject(object, insertIndex, customIgnoreDuplicates, customReplaceDuplicate) {
    if (customIgnoreDuplicates === void 0) {
      customIgnoreDuplicates = false;
    }
    if (customReplaceDuplicate === void 0) {
      customReplaceDuplicate = undefined;
    }
    if (preventDuplicates && !customIgnoreDuplicates) {
      var existingIndex = history.findIndex(function (e) {
        return e[field] === object[field];
      });
      if (existingIndex !== -1) {
    ...
    var previousObject = previous();
    var objectType = previousObject && previousObject.type === 'primary' ? 'secondary' : 'primary';
    var objectWithDefaultType = _extends({}, object, !object.type && {
      type: objectType
    });
    if (insertIndex !== undefined) {
      insertObjectAtIndex(objectWithDefaultType, insertIndex);
    } else {
      setHistory([].concat(history, [objectWithDefaultType]));
      setCurrentIndex(history.length);
    }
  };
  var updateCurrent = function updateCurrent(updatedObject) {
    if (history.length === 0) return;
    if (currentIndex === 0 && updatedObject.type === 'secondary') {
      updatedObject.type = 'primary';
    }
    var updatedArray = [].concat(history);
    updatedArray[currentIndex] = updatedObject;
    setHistory(updatedArray);
  };
  var updateObjectById = function updateObjectById(updatedObject) {
    if (history.length === 0) return;
    if (currentIndex === 0 && updatedObject.type === 'secondary') {
      updatedObject.type = 'primary';
    }
    var index = history.findIndex(function (e) {
      return e[field] === updatedObject[field];
    });
    if (index !== -1) {
      var updatedArray = [].concat(history);
  ...
  var removeObjectByIndex = function removeObjectByIndex(removeIndex) {
  ...
  var removeObjectByName = function removeObjectByName(removeName) {
    var removeIndex = history.findIndex(function (e) {
      return e[field] === removeName;
    });
    removeObjectByIndex(removeIndex);
  };
  var removeAtIndex = function removeAtIndex(index) {
    if (index >= 0 && index < history.length) {
      var newArr = [].concat(history);
  ...
  var removeAfterIndex = function removeAfterIndex(index) {
  ...
  return {
    ...
    insertObjectAtIndex: insertObjectAtIndex,
    ...
    goBackToPrimary: goBackToPrimary,
    updateObjectById: updateObjectById,
    goForwardToType: goForwardToType,
...
var dataTransformer = function dataTransformer(data, name, obj) {
  return function (local) {
    ...
    if (typeof data === "string") {
      ...
      patternResult = patternResult.replace(/\$\{(.*?)\}/g, function (w, name) {
        var result = _.get(values, name) || '';
...
function createFormControlV4(props) {
                  ...
                  if (_f.formId === formId || formId === "ALL") {
                    return Promise.resolve(validateField(field, get(_formValues, _f.name), isValidateAllFieldCriteria, formOptions.shouldUseNativeValidation)).then(function (fieldError) {
                      if (shouldCheckValid) {
                        ...
                        fieldError[_f.name] ? set(_formState.errors, _f.name, fieldError[_f.name]) : unset(_formState.errors, _f.name);
                        if (context.triggerAll === false && Object.keys(_formState.errors).length == 1) {
                          _interrupt = true;
                        }
                      }
                    });
  ...
  var triggerCustom = function triggerCustom(options, name) {
    if (options === void 0) {
      options = {
        triggerAll: false
      };
        ...
        if (formOptions.resolver) {
          ...
          var _temp19 = function () {
            if (name) {
              ...
              return Promise.resolve(validateForm(_fields, false, {
    ...
    try {
      var _temp24 = function _temp24() {
        if (options.shouldFocus && !isValid) {
          focusFieldBy(_fields, function (key) {
            return get(_formState.errors, key);
          }, name ? _fieldNames3 : _names.mount);
        }
        _proxyFormState.isValid && _updateValid();
        return isValid;
      };
      var _fieldNames3 = convertToArrayPayload(name);
      var isValid;
      console.log("dyno ;)", "trigger", _formState.errors);
      var _temp23 = function () {
        if (formOptions.resolver) {
          return Promise.resolve(executeResolverValidation(isUndefined(name) ? name : _fieldNames3)).then(function (schemaResult) {
            isValid = name ? _fieldNames3.every(function (name) {
              return !get(schemaResult, name);
            }) : isEmptyObject(schemaResult);
          });
        } else {
          var _temp22 = function () {
            if (name) {
              return Promise.resolve(Promise.all(_fieldNames3.map(function (fieldName) {
                try {
                  var _ref7;
                  var field = get(_fields, fieldName);
                  return Promise.resolve(validateForm(field._f ? (_ref7 = {}, _ref7[fieldName] = field, _ref7) : field));
                } catch (e) {
                  return Promise.reject(e);
                }
              }))).then(function (_Promise$all3) {
                isValid = _Promise$all3.every(Boolean);
              });
            } else {
              return Promise.resolve(validateForm(_fields, true)).then(function (_validateForm5) {
  ...
  var triggerBackgroundOptimised = function triggerBackgroundOptimised(formId) {
    ...
    return function (list) {
      ...
      try {
            ...
            var _temp25 = function () {
              if (list) {
                ...
                return Promise.resolve(validateForm(_fields, true, {
                  valid: true
                }, formId)).then(function (_validateForm7) {
                  isValid = _validateForm7;
                });
              }
            }();
        ...
        return Promise.resolve(_temp26 && _temp26.then ? _temp26.then(_temp27) : _temp27(_temp26));
      } catch (e) {
        return Promise.reject(e);
      }
    };
  ...
  var register = function register(name, options) {
    ...
    return isWindowUndefined$1 ? {
      ...
      ref: function ref(_ref8) {
        if (_ref8) {
          registerFieldRef(name, _ref8, options);
        } else {
          var _field = get(_fields, name, {});
          var _shouldUnregister = formOptions.shouldUnregister || options.shouldUnregister;
          if (_field._f) {
        ...
        return Promise.resolve(_finallyRethrows(function () {
          return _catch(function () {
            function _temp31() {
              var _temp29 = function () {
                if (isEmptyObject(_formState.errors) && Object.keys(_formState.errors).every(function (name) {
                  ...
                  var _temp28 = function _temp28(_onInvalid) {
                    _onInvalid;
                    formOptions.shouldFocusError && focusFieldBy(_fields, function (key) {
                      return get(_formState.errors, key);
                    }, _names.mount);
                  };
                  return onInvalid ? Promise.resolve(onInvalid(_formState.errors, e)).then(_temp28) : _temp28(onInvalid);
                }
              }();
            ...
            var _temp30 = function () {
              if (formOptions.resolver) {
                return Promise.resolve(executeResolver()).then(function (_ref9) {
...
var FormBuilderNext$1 = React__default.forwardRef(function (_ref7, ref) {
  ...
  console.log("dyno ;)", defaultValues, "defaultValues", _extends({}, proxyDefaultValues), dataStore);
  ...
  var getValuesPOC = function getValuesPOC(options) {
    if (options === void 0) {
      options = {
        triggerAll: false
      };
    ...
    try {
      if (Object.keys(errors).length > 0) return Promise.resolve(false);
      return Promise.resolve(triggerCustom(options)).then(function (result) {
    ...
    getValuesBackground: function getValuesBackground(validation) {
      ...
      try {
        var _temp7 = function _temp7(_result5) {
          return _exit4 ? _result5 : Promise.resolve(getValues());
        };
        var _exit4 = false;
        var _temp6 = function () {
          if (validation) {
    ...
    try {
      ...
      var _temp8 = function () {
        if (hasCondition !== undefined) {
          return Promise.resolve(hasCondition.map(function (item) {
            try {
              var _temp10 = function _temp10(touched) {
                var i = n[item.refId];
                console.log("dyno ;)", n["accountNo"], "accountNoaccountNo", '-----', i);
                if (i !== undefined && i.visible !== touched) {
                  n[item.refId].visible = touched;
              ...
              var realValue = value && value["value"] || value;
              var _item$type = item === null || item === void 0 ? void 0 : item.type;
              return Promise.resolve(_item$type ? Promise.resolve(validationResolver[item.type](item, realValue)).then(_temp10) : _temp10(_item$type));
            } catch (e) {
              return Promise.reject(e);
            }
          })).then(function () {});
...
exports.useDynamoHistory = useDynamoHistory;
